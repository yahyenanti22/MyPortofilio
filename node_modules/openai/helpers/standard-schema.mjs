...
function bindStandardSchema(standardSchema) {
    let binding;
    return () => {
        if (!binding) {
...
function parseStandardSchema(getBinding, content) {
...
function resolveStandardJSONSchema(getBinding, schemaOverride) {
    const schema = (schemaOverride ?? getBinding().standard.jsonSchema?.input({ target: 'draft-07' }));
    if (!schema) {
